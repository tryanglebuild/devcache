  ...
  if (items.length === 0) {
    return (
      <div className="text-center py-12 bg-white rounded-xl shadow-[0_8px_32px_-4px_rgba(25,28,30,0.06)]">
        ...
        <p className="text-[#464554] font-medium">No favorite items yet</p>
      </div>
    )
  }
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {items.map((item) => (
        <button
          ...
          className="bg-white p-4 rounded-xl shadow-[0_8px_32px_-4px_rgba(25,28,30,0.06)] hover:shadow-lg transition-all cursor-pointer text-left"
        >
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <h4 className="font-bold text-[#191c1e] truncate">{item.name}</h4>
                <span className="material-symbols-outlined text-[#904900] text-sm flex-shrink-0" style={{ fontVariationSettings: "'FILL' 1" }}>
                  star
                </span>
              </div>
              {item.description && (
                <p className="text-xs text-[#464554] line-clamp-2">{item.description}</p>
              )}
