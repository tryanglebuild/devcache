...
export function EditTagModal({ isOpen, onClose, tag, onTagUpdated }: EditTagModalProps) {
  ...
  useEffect(() => {
    setName(tag.name); setDescription(tag.description || ''); setColor(tag.color); setCustomColorInput(tag.color)
  }, [tag])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error } = await supabase
        .from('user_tags')
        .update({ name: name.trim().toLowerCase(), description: description.trim() || null, color })
        .eq('id', tag.id)
        .select().single()
      if (error) {
