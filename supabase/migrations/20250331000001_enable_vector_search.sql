-- Enable Vector Search for Chat System
-- This migration adds vector similarity search capabilities

-- Ensure vector extension is enabled
CREATE EXTENSION IF NOT EXISTS vector;

-- Add vector column to agent_template_embeddings if not exists
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'agent_template_embeddings' 
    AND column_name = 'embedding_vector'
  ) THEN
    ALTER TABLE agent_template_embeddings 
    ADD COLUMN embedding_vector vector(1536);
  END IF;
END $$;

-- Create index for vector similarity search (HNSW for better performance)
CREATE INDEX IF NOT EXISTS idx_agent_template_embeddings_vector 
ON agent_template_embeddings 
USING hnsw (embedding_vector vector_cosine_ops);

-- Add vector column to project_items for semantic search
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
...
-- Create index for project_items vector search
CREATE INDEX IF NOT EXISTS idx_project_items_embedding_vector 
ON project_items 
USING hnsw (embedding_vector vector_cosine_ops);

-- Create hybrid search function combining vector + text search
...
-- Add comment
COMMENT ON FUNCTION search_resources_hybrid IS 'Hybrid search combining vector similarity and text search for chat recommendations';
